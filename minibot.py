...
if (config["testing_mode_enabled"]):
    # If the testing mode is enabled in the config we will set the debug scope to the guild ID specified in the config and enable the deletion of unused application commands.
    print("Testing mode enabled.\nSlash commands will be automatically instantiated with guild ID scope specified in config.")
    client = interactions.Client(token=config["token"], intents=Intents.DEFAULT, delete_unused_application_cmds=True, debug_scope=config["testing_guild_id"])
else:
    # If the testing mode is disabled in the config we will setup the client as normal.
    client = interactions.Client(token=config["token"], delete_unused_application_cmds=False, intents=Intents.DEFAULT)
